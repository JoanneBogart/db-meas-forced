...
class Algo_base_CircularApertureFlux(algobase.Algo):
    fluxes = [
        {
            "flux": 'base_CircularApertureFlux_{}_flux'.format(size),
        }
    ...
    fluxerrs = [
        {
            "flux": 'base_CircularApertureFlux_{}_flux'.format(size),
            "fluxerr": 'base_CircularApertureFlux_{}_fluxSigma'.format(size),
        }
        for size in ["3_0", "4_5", "6_0", "9_0", "12_0", "17_0", "25_0", "35_0", "50_0", "70_0"]
    ]

    renamerules = [
        (r'base_Circular(ApertureFlux_)([0-9]+)_([0-9]+)(_)', _rename_aperture_size),
        (r'base_Circular', ''),
    ]

    def __init__(self, sourceTable):
        self.sourceTable = sourceTable.cutout_subtable("base_CircularApertureFlux_")
